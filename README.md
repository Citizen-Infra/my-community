...
Secondary deployments can set `VITE_PINNED_COMMUNITY_ID` to preselect one community and hide the picker, `VITE_BLUESKY_ENABLED=false` to remove Network, Bluesky sign-in, and OAuth client metadata, and `VITE_LINKS_API_BASE` to read links from that community's private service. The pinned PXXI deployment always disables Bluesky, offers Telegram sign-in first and email for accounts already linked to Telegram, and routes the Community Input sign-in prompt to those same account options. On PXXI, `VITE_DECISIONS_API_BASE` adds Crapotkin's member-gated decision index to the existing Community Input feed and provides the selected decision's detail route; PXXI falls back to its configured links origin because both endpoints live on Crapotkin. Other deployments never load the private brain index. `VITE_WEB_ORIGIN` remains the deployment's exact public origin. `web/.env.philanthropic-xxi.example` records the PXXI shape without credentials.
