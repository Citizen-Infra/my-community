...
Secondary deployments can set `VITE_PINNED_COMMUNITY_ID` to preselect one community and hide the picker, `VITE_BLUESKY_ENABLED=false` to remove Network, Bluesky sign-in, and OAuth client metadata, and `VITE_LINKS_API_BASE` to read links from that community's private service. `VITE_DECISIONS_API_BASE` enables the dedicated member decision reader; the PXXI deployment falls back to its configured links origin because both endpoints live on Crapotkin. `VITE_WEB_ORIGIN` remains the deployment's exact public origin. `web/.env.philanthropic-xxi.example` records the PXXI shape without credentials.
