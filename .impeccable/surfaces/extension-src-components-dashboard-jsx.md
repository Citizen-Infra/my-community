...
**Scope:** Present enabled feeds in an adaptive overview with no redundant main-area page heading. Four feeds form a 2×2 mosaic; fewer feeds expand to use the space. Each tile auto-fits as many complete, civic-priority preview rows as its available height allows, with a locally persisted exact-count override. Visible rows divide the tile body evenly, becoming multiline editorial summaries when sparse and compact scan lines when dense. Rows deep-link to their item; headings and footers open the complete feed. A compact sliders control beside Dashboard in the sidebar toggles Customize mode, which supports pointer drag, 44px keyboard controls, preview-depth selectors, contextual guidance, and Reset layout. Decorative feed-color bars are removed, tab search recedes on the dashboard, and the Collections sidebar retains its established width so names are not compressed.
