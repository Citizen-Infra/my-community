...
- **Web companion:** preserve the same overview-to-focused-feed rhythm with real paths and browser history. The web top bar names the current communities and synchronization state without becoming a SaaS navigation rail. Settings enters as a warm-paper side sheet on wide screens and a full-width surface on narrow screens. Below 680px, the mosaic becomes one readable vertical sequence; cards keep comfortable text and touch targets rather than shrinking into a miniature desktop grid. Extension-only controls are absent, never disabled placeholders.
