...
- **Community Input:** combines consent decisions and suggested wiki sources, prioritizing items awaiting the member's response. On the pinned Philanthropic XXI deployment it also includes lifecycle-tagged decision records from the private community brain; selecting one opens its member-gated detail view. Harmonica sessions remain in Participation, and no separate Decisions tile is introduced. Members can suggest the current page to a selected community's wiki queue.
